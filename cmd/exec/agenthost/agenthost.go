...
// RunURLEnvVar carries ConversationInfo.RunURL to a host-CLI invocation, which
// resolves its identity from the environment rather than from a daemon's
// per-socket info. Set by the spawner beside the conversation id; unset
// leaves the footer linkless.
const RunURLEnvVar = "TRIAGE_FACTORY_RUN_URL"

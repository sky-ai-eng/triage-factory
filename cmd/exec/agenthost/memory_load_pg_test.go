...
// reading run's owning team into the memory read (admin-pool, BYPASSRLS): on one
// SHARED org-wide entity, a team-1 run sees its own team-visible memory plus any
...
func TestLocalClient_MemoryLoad_Postgres_TeamScoped(t *testing.T) {
	...
	// excluded from team-1's read); an org-visible run owned by team-2 (included
	// via the visibility='org' arm, NOT the team arm).
	seedPgRunWithMemory(t, h, stores, orgID, team1, owner, entityID, "team", "team1 narrative")
	seedPgRunWithMemory(t, h, stores, orgID, team2, owner, entityID, "team", "team2 narrative")
	seedPgRunWithMemory(t, h, stores, orgID, team2, owner, entityID, "org", "org-wide narrative")

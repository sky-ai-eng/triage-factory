...
// footer is the disclosure block appended to everything this conversation
// publishes in a human's name — the same shape the server appends when a
// human approves a staged review, so no surface discloses differently. Empty
// with no conversation: a human running the CLI directly gets their body back
// unmodified rather than a "partially generated by AI" line that would be a
// lie.
func (c *LocalClient) footer(kind string) string {
	if c.info.ConversationID == "" {
		return ""
	}
	return agentmeta.Build(kind, c.info.RunURL)
}
...
// GithubCreatePR opens the pull request with the disclosure footer already on
// its body, so the draft carries it from its first second on GitHub — a reader
// never sees an undisclosed agent-authored description, and approval has no
// reason to rewrite the body later. The footer is static, so a retried create
// (idempotent on the PR number) cannot stack a second one.
...
// breadcrumb — the PR still opened. The `draft` flag selects the artifact's
// initial state (draft vs open);

...
**Scope — self-host `docker-compose` only, not the Fly.io production deployment.** `fly.toml` has no `security_opt`-equivalent field, so this profile doesn't apply there; the hosted deployment's `triagefactory` process still runs with unconfined seccomp inside its Fly Machine (see the comment in `fly.toml`). That's a real gap in coverage, but a narrower one than it sounds: each Fly Machine is already its own Firecracker microVM — a per-tenant hardware isolation boundary, not a kernel shared with other tenants — which is a different and independently stronger property than what self-host's shared Docker engine has going for it. This PR closes the self-host gap; it does not claim to close a Fly-side one.

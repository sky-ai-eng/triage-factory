...
| **Routing Disposition** | `system:routing:disposition` | `Router.HandleEvent` finished handling one event — frozen, taskless (no handler/owner/unroutable), or task created/bumped. Lets an async event source (e.g. Slack) learn synchronously-unavailable routing outcomes (TFAC-593) |

...
behind `loaded && has('<feature>')`** (the `useEntitlements` hook over
`/api/entitlements`) **and degrades to absence** — no upsell stubs or
disabled placeholders — matching the backend's 404-and-hide non-disclosure

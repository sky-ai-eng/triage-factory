...
`event_queue`, the router's durable event queue, `pending_firings`, the
per-task queue of auto-delegation intents the firing worker drains, and
`task_rederive_queue`, the post-scoring re-evaluation of each task's deferred
triggers that the re-derive worker drains. Every label is a closed vocabulary
or an opaque id: the kind name, the org id, the typed attempt outcome, the
park reason, the verb.

Counters are per process, incremented by whichever process committed the
disposition: the process running the kind's worker (for all three kinds, the

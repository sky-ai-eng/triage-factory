...
- **Installed but not yet connected.** If an organization *member* without owner rights picks that organization, GitHub sends an install request to an owner instead of installing, and TF shows `Install request sent`; once the owner approves, the admin presses Connect again. Someone who installs from the App's public page on GitHub, outside any workspace, lands on TF's `/github/installed` page, which asks them to sign in and pick the workspace to connect it to. Both are ordinary states, not faults.

...
`TF_MAX_CONCURRENT_RUNS` agents at once — **default 4, and the cap applies in
...
```bash
export TF_MAX_CONCURRENT_RUNS=8   # each concurrent run costs ~256 MB RAM plus API spend
```

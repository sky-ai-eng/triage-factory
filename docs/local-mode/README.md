...
- [Environment tuning](tuning.md) — run concurrency (`TF_MAX_CONCURRENT_CLAIMS`), logging, tracing (`TF_TRACES_ENDPOINT` + a one-container Tempo), `TF_CLAUDE_BINARY`, agent-engine JIT

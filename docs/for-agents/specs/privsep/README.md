...
customer/CISO-facing companion is `docs/security/security-overview.md`; this
...
(`docs/for-agents/specs/horizontal-scaling/`, TFAC-71).
...
- **control vs. executor** — horizontal split across hosts (which machine does
  which job). Scale. `docs/for-agents/specs/horizontal-scaling/`.
...
capability boundary — see `docs/security/security-overview.md` §6 vector 4.
...
`docs/security/security-overview.md` §4.3 and sandbox-fleet §4.
...
- Container cap **grant** is unchanged — privsep shrinks who *wields* caps, not
  ...
  by topology (`docs/security/security-overview.md` §7), not this epic.

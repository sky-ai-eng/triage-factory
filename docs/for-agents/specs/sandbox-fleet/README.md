...
(`docs/for-agents/specs/playwright-chromium-sandbox/`) — a "browser" profile becomes *one
...
SaaS" means Tier 2 in `docs/security/isolation-tiers.md`.
...
- **Genuinely private network, no public route** (e.g. a k8s stack on a private
  ...
  `docs/security/isolation-tiers.md` already draws (the strict-isolation need and the
...
Already specced in `docs/for-agents/specs/playwright-chromium-sandbox/`. The fleet
...
`docs/for-agents/specs/horizontal-scaling/` §6.4 for the cross-tenant invariant), while
...
- External-URL rendering for the browser profile (would route through the same
  gating proxy — `docs/for-agents/specs/playwright-chromium-sandbox/` §3 flags it).

---

## Related

- `docs/for-agents/specs/horizontal-scaling/` — §6.4: how profiles interact with the
  executor fleet (eligibility vs affinity in the claim, budget admission,
  cross-org variant sharing).
- `docs/for-agents/specs/playwright-chromium-sandbox/` — the browser profile; the first
  worked instance of the image dimension.
- `docs/security/isolation-tiers.md` — the tier ladder the egress power-dial and the
  public-vs-private split slot into.
- `docs/for-agents/specs/sky-254-runsc-validation/` — the sandbox + the proxy/egress
...
- `internal/sandbox/doc.go` — the Property-A/B credential invariants §7 lives
  inside.
- `docs/security/security-overview.md` — the sandbox threat model, the exact

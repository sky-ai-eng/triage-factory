...
#
# Rotating the prod key is deliberate: update ee/ee.go's default AND EXPECTED here in
# the same change, then reissue customer tokens. Source of truth:
# `licensectl pubkey --env prod` (tf-internal ops repo).
set -euo pipefail
...
KEY="$(sed -n 's/^var publicKeyB64 = "\(.*\)"$/\1/p' ee/ee.go)"

if [ -z "$KEY" ]; then
  echo "::error::could not extract a non-empty publicKeyB64 default from ee/ee.go — builds would ship with enterprise features DISABLED. Restore the tf-license-prod public key as the source default." >&2
  exit 1
fi

if [ "$KEY" != "$EXPECTED" ]; then
  echo "::error::ee/ee.go's publicKeyB64 default does not match the expected tf-license-prod public key — likely a mangled edit (truncated paste / stray whitespace) or the wrong key. Builds would silently ship EE-disabled. If the prod key legitimately rotated, update the ee/ee.go default AND EXPECTED in scripts/verify-license-key.sh in the same change." >&2
  exit 1
fi
...
if ! echo "$KEY" | base64 -d 2>/dev/null | openssl pkey -pubin -inform DER -noout 2>/dev/null; then
...
echo "OK: ee/ee.go's publicKeyB64 source default matches the expected tf-license-prod public key and parses."

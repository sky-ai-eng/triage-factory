...
func (s *Spawner) handleCancelled(orgID, runID string, startTime time.Time, wtPath, triggerType, creatorUserID string) {
	...
	if triggerType == "manual" {
		completeErr = s.tx.SyntheticClaimsWithTx(bgCtx, orgID, creatorUserID, func(ts db.TxStores) error {
			return ts.AgentRuns.Complete(bgCtx, orgID, runID, "cancelled", 0, elapsed, 0, 0, "cancelled", "Cancelled by user", "", "", "")
		})
	} else {
		completeErr = s.agentRuns.CompleteSystem(bgCtx, orgID, runID, "cancelled", 0, elapsed, 0, 0, "cancelled", "Cancelled by user", "", "", "")
	}
	if completeErr != nil {

...
// event_queue rows the router parked, and the redrive that puts them back.
// The routes keep their /api/events/failed/* paths and wire shape; the rows
// behind them are the queue's parked rows.
//
// Why the pair is worth a UI at all: a parked row is an event that is
// durably recorded but whose routing obligations — task mint/bump, owner
// resolution, trigger firing, close processing — spent the attempt budget
// or failed permanently, and will never run on their own. Nothing
// re-drives it. The tracker's snapshot advanced when the event was minted,
// so the transition that produced it does not re-emit, and the row is
// retained forever precisely because a human is the only remaining
// recovery path.
//
...
type failedEventsHandler struct {
	az *authz.Checker
	// queue is the admin-pool EventQueueStore. Only the org-scoped operator
	// methods are called from here; the drain worker owns the rest.
...
// open doesn't display a frozen "3m ago". attempts is the row's charged
// attempt count and enqueued_at its original enqueue, preserved across a
// redrive.
...
func failedEventToJSON(e domain.ParkedEvent) failedEventJSON {
	return failedEventJSON{
		...
		EnqueuedAt:     e.FirstEnqueuedAt.UTC().Format(time.RFC3339),
	}
}
...
func (h *failedEventsHandler) handleFailedEventsList(w http.ResponseWriter, r *http.Request) {
	...
	rows, total, err := h.queue.ListParked(r.Context(), orgID, db.ListOpts{Limit: page.Limit, Offset: page.Offset, CountOnly: page.CountOnly})
	if err != nil {
...
// the same shape the list serves it. A row in any other status (ready,
// leased, done, cancelled) is 404 rather than 200: the resource this route
// addresses is the parked set, and answering with a row the list would never
// show would make the two reads disagree about what "a failed event" is.
func (h *failedEventsHandler) handleFailedEventGet(w http.ResponseWriter, r *http.Request) {
	...
	row, err := h.queue.GetParked(r.Context(), orgID, id)
	if err != nil {
		internalError(w, "failed-events", err)
		return
	}
...
// failedEventsRequeueRequest is the redrive body: the queue ids an operator
...
// handleFailedEventsRequeue serves POST /api/events/failed/requeue, the
// operator redrive: each named parked row returns to ready with a fresh
// budget.
//
// Redriving is safe to expose to an operator because it changes nothing
...
// fence and the one-active-run index. So a double-redrive, or a redrive of an
...
func (h *failedEventsHandler) handleFailedEventsRequeue(w http.ResponseWriter, r *http.Request) {
	...
	if len(req.IDs) > db.MaxRedriveIDs {
		httpx.WriteErrors(w, http.StatusBadRequest, httpx.ErrorItem{
			Reason:  httpx.ReasonOutOfRange,
			Message: fmt.Sprintf("at most %d ids per requeue", db.MaxRedriveIDs),
			Field:   "ids",
		})
		return
	}

	n, err := h.queue.Redrive(r.Context(), orgID, req.IDs, ClaimsFrom(r.Context()).Subject)
	if err != nil {
		internalError(w, "failed-events", err)
		return
	}
	// Worth a line at info: it is a human deliberately re-driving parked
	// work, and the next thing in the log is that work routing.
	failedEventsLog.InfoContext(r.Context(), "operator redrove parked event rows",

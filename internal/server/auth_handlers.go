...
func (s *Server) handleMe(w http.ResponseWriter, r *http.Request) {
	...
	// The request's active org, read from context rather than off the session
	// row: withSession puts a session's cursor and a token's sealed org in the
	// same place, so this answers for both credentials. Reaching into the
	// session here would tell a Bearer caller they have no active org — on the
	// one endpoint every client reads to find out.
	resp.ActiveOrgID = OrgIDFrom(r.Context())
...
func (s *Server) handleMeIdentities(w http.ResponseWriter, r *http.Request) {
	...
	// Same as /api/me: the org comes from request context, which carries a
	// session's cursor or a token's sealed org alike.

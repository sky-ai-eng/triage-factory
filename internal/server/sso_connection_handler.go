...
//
// The createSAMLProvider GoTrue admin-API helper below is re-homed from the
// scrapped startup bootstrap (claude/cool-davinci-jahpyk); it was the good part.
// What changed is the trigger (an org-admin request, not boot) and the
// idempotency key (the caller org's existing connection row, not a domain match
// — domains are out of scope here, TFAC-428).
//
// Admin-API auth: TF presents a pre-minted, long-lived RS256 service-role token
// (TF_GOTRUE_SERVICE_ROLE_TOKEN, generated by `triagefactory jwk-init`) as a
// static bearer. TF holds ONLY this token, never GoTrue's signing key, and mints
// nothing. This supersedes the spike's HS256 approach: under the production
// GOTRUE_JWT_ALGORITHM=RS256 config, GoTrue 403s an HS256 token signed with
// GOTRUE_JWT_SECRET ("signing method HS256 is invalid") and accepts only an
// RS256 token carrying role:service_role (verified empirically against
// supabase/gotrue:v2.189.0). GoTrue does not enforce aud/iss on the admin token.

// envServiceRoleToken names the env var holding the pre-minted RS256 service-role
// JWT TF presents to GoTrue's admin SSO API. `triagefactory jwk-init` mints it
// (signed with the same RSA key as GOTRUE_JWT_KEYS) and the compose stack passes
// it to the TF service. Optional-but-required-for-SSO: a deployment that never
// registers an SSO connection need not set it. Treat it as a secret.
...
func (h *ssoConnectionHandler) handleSSOConnectionCreate(w http.ResponseWriter, r *http.Request) {
	...
	// Register the provider with GoTrue's admin SSO API, authenticated with the
	// pre-minted RS256 service-role token (TF mints nothing — see the file doc).
	// The token is optional-but-required-for-SSO: when it's absent the deployment
	// hasn't been set up for SSO, so return a clear, actionable operator error

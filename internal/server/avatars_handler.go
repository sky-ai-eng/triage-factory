...
// Avatar proxy. User/member avatars come from OAuth metadata (users.avatar_url
// — GitHub's avatars.githubusercontent.com, the org's Jira host, and so on),
// all of them cross-origin. The app's CSP is `img-src 'self' data:`
// (security_headers.go), so a direct cross-origin <img> is blocked as a CSP
// violation and the picture never loads.
//
// Rather than widen img-src to a per-deployment allowlist of provider CDNs,
// the image is proxied through this same-origin endpoint: the browser only
// ever loads /api/avatars/{id}, and the server fetches + caches the upstream
// image. That keeps the CSP to `'self'` and out of org settings, which would
// otherwise have to supply the Jira avatar host.
...
const (
	...
	// avatarSuccessTTL is how long a fetched image is served from cache before
	// a re-fetch. Avatars change rarely, and a re-login refreshes the stored
	// URL anyway, so a long TTL is fine — at most it serves a picture ~6h
	// stale.
	avatarSuccessTTL = 6 * time.Hour
	// avatarFailureTTL negative-caches a failed fetch (unreachable host,
	// non-image body, SSRF reject) briefly, so a roster full of broken avatars
	// can't make the 15s-polling Usage page hammer the upstream (or us) every
	// cycle.
...
// never reassigns them. The cache enforces TWO independent bounds, because
// either alone is insufficient:
//   - avatarCacheMaxBytes caps total memory. An entry-count cap alone,
//     times the per-image maxAvatarBytes (2 MiB), would permit multi-GiB
//     of resident data.
//   - avatarCacheMaxEntries caps the map size. A flood of distinct FAILED
//     urls fills the cache with zero-byte negative entries the byte budget
//     cannot evict.
var (
...
// The handler maps it to a 502 and the frontend Avatar falls back to a
// monogram.
var errAvatarUnavailable = errors.New("avatar unavailable")

// errBlockedAvatarHost marks a fetch rejected by the dial-time SSRF guard (a
// non-public or non-IP resolved address). It is wrapped through the dial
// Control hook so the fetch path can errors.Is it and log at Warn — a
// private-IP avatar url is a probe signal, not a routine broken image. The
// chain survives the net stack, since net.OpError and url.Error both Unwrap.
...
// address — outside loopback/link-local/multicast/unspecified
// (IsGlobalUnicast), RFC 1918 / ULA (IsPrivate), and the
// CGNAT/doc/benchmark/reserved ranges in nonPublicPrefixes (shared with the
// GitHub manifest reachability check). It is the single SSRF predicate behind
// both the manifest pre-check and the avatar proxy's dial-time guard.
...
// process lifetime. Concurrent fetches of the same URL collapse via
// singleflight; results are TTL-cached, success and failure alike.
type avatarFetcher struct {
	...
	// guarded by mu — the running figure cachePut keeps under
	// avatarCacheMaxBytes.
	totalBytes int64
}
...
// evicts soonest-to-expire entries until BOTH bounds hold for the incoming
// entry: total bytes within avatarCacheMaxBytes (memory) and entry count
// within avatarCacheMaxEntries (map size, the bound that catches a flood of
// zero-byte negative entries). A single image larger than the whole budget
// still stores once into an otherwise-empty cache — the loop never spins on an
// empty map.
func (f *avatarFetcher) cachePut(key string, img *fetchedAvatar) {
	...
	// Drop any existing entry for this key first, so its old size doesn't
	// linger in totalBytes and the count below reflects the post-insert state.
...
// shared fetch. Both success and failure are cached. Returns
// errAvatarUnavailable for a cached failure.
func (f *avatarFetcher) fetch(rawURL string) (*fetchedAvatar, error) {
	...
	v, err, _ := f.group.Do(rawURL, func() (any, error) {
		...
		if ferr != nil {
			...
			// singleflight + negative cache naturally rate-limit it to ~once
			// per fetch attempt. A dial-time SSRF rejection is a probe signal
			// (Warn) — e.g. a private IP hand-inserted into users.avatar_url;
			// a routine broken avatar is just noise (Debug). Then
			// negative-cache and return the generic sentinel, so first-failure
			// and cached-failure callers see the same error.
...
// doFetch performs one upstream GET: https-only, size-capped, and
// content-type-validated. The SSRF IP guard lives on the client's dialer (so
// it covers redirect hops too); the scheme is checked here for the initial
// request.
...
// (http.DetectContentType) — deliberately NOT the upstream Content-Type
// header. We re-serve third-party content from our own origin, so a
// spoofed/incorrect header must not decide the type the browser sees; the
// bytes must actually be an allowed raster image. Returns the sniffed type
// when it's allowed, else "" (the caller rejects the fetch). The concrete type
// also matters because the global X-Content-Type-Options: nosniff makes our
// declared type authoritative.
...
// the app's `img-src 'self'` CSP.
//
// Gate: any authenticated org member (the session wrap + requireOrg). The
// target user's avatar_url is resolved on the APP pool under the caller's
// claims, so users_select RLS scopes visibility to the caller's own row +
// co-org-members — a cross-org {user_id} resolves to no row (empty avatar →
// 404), never a leak. An absent/empty avatar 404s too, so the frontend Avatar
// shows its monogram.
//
...
func (h *avatarsHandler) handleAvatar(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// Expected failure mode (unreachable host, non-image body, SSRF
		// reject). fetch() already logged the specific cause (Warn for an SSRF
		// rejection, Debug otherwise), rate-limited by its cache; the frontend
		// Avatar's onError falls back to a monogram, so just 502 here.
	...
	// Authz-gated, so private (never a shared/CDN cache). A short browser
	// cache keeps the 15s Usage poll + re-renders from re-hitting this
	// endpoint.

...
// this run via TRIAGE_FACTORY_CONVERSATION_ID → ResolveRunIdentity, and the
// server path is system-driven).
func Build(agentRuns db.ConversationStore, orgID, runID, kind string) string {
	if runID == "" {
		return ""
	}
...
func elapsedFromRun(ctx context.Context, agentRuns db.ConversationStore, orgID, runID string, run *domain.Conversation) string {
...
func costFromRun(ctx context.Context, agentRuns db.ConversationStore, orgID, runID string, run *domain.Conversation) (cost float64, prefix string) {

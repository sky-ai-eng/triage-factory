...
func RunClaimTakeoverConformance(t *testing.T, mk ClaimLeaseFactory) {
	...
	t.Run("Takeover_RefusesTheHoldersLateRenewal", func(t *testing.T) {
		...
		if _, err := f.Stores.ConversationQueue.RenewClaimLeaseSystem(ctx, f.OrgID, c.ID, c.ClaimID, testClaimLease, db.ClaimActivity{}); !errors.Is(err, db.ErrClaimReleased) {

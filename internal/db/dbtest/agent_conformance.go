...
// RunAgentStoreConformance. The factory returns the wired store + the orgID
// to pass to every method. The Postgres backend pre-seeds the orgs row + the
// owner users row so Create's FKs satisfy; SQLite runs against the local
// sentinel org.
...
//   - Invalid-UUID ids: SetGitHubOrgIdentity refuses with db.ErrNoSuchAgent
//     rather than bubbling a 22P02 parse error (Postgres) or silently
//     affecting zero rows (SQLite).
...
func RunAgentStoreConformance(t *testing.T, factory AgentStoreFactory) {
	...
	t.Run("every_single_row_write_returns_the_stored_row", func(t *testing.T) {
		// The returned-row standard applied to the store's one id-keyed write.
		...
		// The row carries what the write's arguments say nothing about, which
		// is why an input struct cannot stand in for it.
		if half.DisplayName != "Returned Bot" {
			t.Errorf("SetGitHubOrgIdentity returned display name %q, want the row's", half.DisplayName)
		}
		...
		// A miss is an error rather than a silent no-op.

...
type ConversationSeeder struct {
	...
	// state ('pending' | 'written' | 'failed'). The key is the task, which is
	// what the workspace is keyed by. WorkspaceSnapshotStore owns that table,
	// so it is a seeded precondition here rather than a store call — the
	// eviction enumeration reads it as its safety gate, and the suite has to
	// be able to stage all three states plus the no-row case.
...
//
//   - Lifecycle methods (the holder's terminal and park, SetSession,
//     MarkQueuedForResume, the failure terminal)
...
func RunConversationStoreConformance(t *testing.T, mk ConversationStoreFactory) {
	...
	t.Run("Complete_SkipsSyntheticRowsWhenSettling", func(t *testing.T) {
		...
		if _, err := HolderComplete(store, ctx, orgID, conversationID, "failed", 1.5, 1000, 2, "", "", "", "infra"); err != nil {
			t.Fatalf("Complete: %v", err)
		}
		msgs, err := store.MessagesForConversations(ctx, orgID, []string{conversationID})
		if err != nil {
		...
		if _, err := HolderComplete(store, ctx, orgID, onlySynthID, "failed", 0.25, 0, 0, "", "", "", "infra"); err != nil {
	...
	t.Run("Complete_ConversationWideFallback_RanksRealModelThenNoModel", func(t *testing.T) {
		...
		if _, err := HolderComplete(store, ctx, orgID, noRealID, "failed", 0.5, 0, 0, "", "", "", "infra"); err != nil {
	...
	t.Run("CancelAndFail_TokensStillDeriveFromLedger", func(t *testing.T) {
		...
		t.Run("ParkOpen_stopped", func(t *testing.T) {
			...
			ok, err := HolderPark(store, context.Background(), orgID, conversationID, db.ParkStopped(domain.ParkReasonUserCancelled, "cancelled"))
			if err != nil || !ok {
				t.Fatalf("park stopped: ok=%v err=%v", ok, err)
			}
		...
		t.Run("MarkFailedIfActive", func(t *testing.T) {
			store, orgID, _, seed := mk(t)
			conversationID := seedRunningWithTokens(t, store, orgID, seed)
			ok, err := HolderMarkFailed(store, context.Background(), orgID, conversationID, "")
			if err != nil || !ok {
	...
	t.Run("MarkFailedIfActive_PersistsFailureKind", func(t *testing.T) {
		...
		conversationID := seedConversationForTest(t, orgID, seed, "running")
		ok, err := HolderMarkFailed(store, ctx, orgID, conversationID, string(domain.ConversationFailureMemoryLimit))
		if err != nil || !ok {
		...
		if ok, err := HolderMarkFailed(store, ctx, orgID, plainID, ""); err != nil || !ok {
	...
	t.Run("Complete_PersistsFailureKind", func(t *testing.T) {
		...
		conversationID := seedConversationForTest(t, orgID, seed, "running")
		if _, err := HolderComplete(store, ctx, orgID, conversationID, "failed", 0, 0, 0, "", "", "", string(domain.ConversationFailureAgentError)); err != nil {
		...
		plainID := seedConversationForTest(t, orgID, seed, "running")
		if _, err := HolderComplete(store, ctx, orgID, plainID, "failed", 0, 0, 0, "", "", "", ""); err != nil {
	...
	t.Run("MarkOpen_FlipsRunning_RefusesTerminal", func(t *testing.T) {
		...
		// Running → ok=true.
		conversationID := seedConversationForTest(t, orgID, seed, "running")
		ok, err := HolderPark(store, ctx, orgID, conversationID, db.ParkIdle())
		if err != nil || !ok {
			t.Fatalf("park on running: ok=%v err=%v", ok, err)
		}
		got, _ := store.Get(ctx, orgID, conversationID)
		if got.Status != "open" {
	...
	t.Run("MarkQueuedForResume_CASGuard", func(t *testing.T) {
		...
		if _, err := HolderPark(store, ctx, orgID, conversationID, db.ParkIdle()); err != nil {
			t.Fatalf("open: %v", err)
		}
		if ok, err := store.MarkQueuedForResume(ctx, orgID, conversationID); err != nil || !ok {
		...
		if _, err := HolderComplete(store, ctx, orgID, failedConversation, "failed", 0, 0, 0, "", "", "", ""); err != nil {
			t.Fatalf("fail: %v", err)
		}
	...
	t.Run("MarkQueuedForResume_WakesAParkedStepOfARunningBlueprint", func(t *testing.T) {
		...
		conversationID, _, _ := seedConversationWithBlueprintForTest(t, orgID, seed, "running")
		if ok, err := HolderPark(store, ctx, orgID, conversationID, db.ParkStopped(domain.ParkReasonUserCancelled, "")); err != nil || !ok {
			t.Fatalf("park: ok=%v err=%v", ok, err)
		}
		if ok, err := store.MarkQueuedForResume(ctx, orgID, conversationID); err != nil || !ok {
	...
	t.Run("MarkQueuedForResume_StampsTheLastEngagementsExecutor", func(t *testing.T) {
		...
		// chase and the row re-queues unowned. Staged parked directly — every
		// park a holder writes names a claim, and this row must have none.
		never := seedConversationForTest(t, orgID, seed, "open")
		if ok, err := store.MarkQueuedForResume(ctx, orgID, never); err != nil || !ok {
			t.Fatalf("MarkQueuedForResume: ok=%v err=%v", ok, err)
		}
	...
	t.Run("MarkFailedIfActive_FailsOpen_RefusesTerminal", func(t *testing.T) {
		...
		if _, err := HolderPark(store, ctx, orgID, openConversation, db.ParkIdle()); err != nil {
			t.Fatalf("open: %v", err)
		}
		ok, err := HolderMarkFailed(store, ctx, orgID, openConversation, "")
		if err != nil || !ok {
	...
	t.Run("ParkOpen_Stopped_ParksActive_RefusesTerminal", func(t *testing.T) {
		...
		ctx := context.Background()
		conversationID := seedConversationForTest(t, orgID, seed, "running")
		ok, err := HolderPark(store, ctx, orgID, conversationID, db.ParkStopped(domain.ParkReasonUserCancelled, "user cancelled"))
		if err != nil || !ok {
		...
		found := false
		for _, k := range keys {
			if k.TaskID == got.TaskID {
				found = true
			}
		}
		if !found {
			t.Errorf("cancel-parked conversation's snapshot key %q is not reapable; its workspace blob would leak forever", got.TaskID)
		}
	...
	t.Run("ParkOpen_Idle_DoesNotRepark", func(t *testing.T) {
		...
		if ok, err := HolderPark(store, ctx, orgID, conversationID, db.ParkIdle()); err != nil || !ok {
		...
		if ok, err := HolderPark(store, ctx, orgID, conversationID, db.ParkStopped(domain.ParkReasonUserCancelled, "stopped")); err != nil || !ok {
	...
	t.Run("ParkOpen_RoundTripsEveryParkReason", func(t *testing.T) {
		...
		for _, reason := range domain.AllParkReasons() {
			conversationID := seedConversationForTest(t, orgID, seed, "running")
			if ok, err := HolderPark(store, ctx, orgID, conversationID, db.ParkStopped(reason, "")); err != nil || !ok {
	...
	t.Run("ListReapableSnapshotKeys_CompletedOnlyBlueprint", func(t *testing.T) {
		...
		// Two steps of ONE blueprint on one task — they share a snapshot key,
		// so the sweep has to reason about the task rather than the row.
		...
		if !reapKeysContain(aged, taskID) {
			t.Errorf("task %s whose every conversation completed is not reapable past the TTL; its workspace blob would never be collected", taskID)
		}
		...
		if reapKeysContain(fresh, taskID) {
			t.Errorf("just-completed task %s is already reapable; the TTL has not elapsed", taskID)
		}
	})
	...
	t.Run("ListReapableSnapshotKeys_FailedConversation", func(t *testing.T) {
		...
		if ok, err := HolderMarkFailed(store, ctx, orgID, conversationID, string(domain.ConversationFailureExecutorLost)); err != nil || !ok {
			t.Fatalf("MarkFailedIfActive: ok=%v err=%v", ok, err)
		}
	...
	t.Run("ListReapableSnapshotKeys_RefusesWhileAnyConversationIsClaimed", func(t *testing.T) {
		...
		if _, err := HolderPark(store, ctx, orgID, parked, db.ParkIdle()); err != nil {
			t.Fatalf("park: %v", err)
		}
	...
	t.Run("ListEvictableWorkspaces_WrittenSnapshotAtRestAndUnclaimed", func(t *testing.T) {
		...
		if got == nil {
			t.Fatalf("parked key %s with a written snapshot is not evictable; its warm tree would be reclaimed only by a restart", wsTask)
		}
		...
		// long enough yet".
		if got := evictableFor(t, store, ctx, time.Now().Add(-time.Hour), wsTask); got != nil {
			t.Errorf("just-parked key %s is already evictable; the idle window has not elapsed", wsTask)
		}
	})
	...
	t.Run("ListEvictableWorkspaces_RefusesUnlessSnapshotIsWritten", func(t *testing.T) {
		...
		for _, state := range []string{domain.WorkspaceSnapshotPending, domain.WorkspaceSnapshotFailed} {
			key := parkedEvictionCandidate(t, store, ctx, orgID, seed, "state-"+state)
			seed.SetSnapshotState(t, key, state)
			if got := evictableFor(t, store, ctx, time.Now().Add(time.Hour), key); got != nil {
				t.Errorf("key %s with snapshot state %q is evictable; only a written blob makes the tree a cache", key, state)
			}
		}
	})

	// A task's conversations share one tree, so any live engagement anywhere
	// under the key is working in the directory this enumeration would hand to
	// a delete.
	t.Run("ListEvictableWorkspaces_RefusesWhileAnyStepIsClaimed", func(t *testing.T) {
		...
		parked, live := mkStep(), mkStep()
		if _, err := store.SetWorktreePathSystem(ctx, orgID, parked, "/tmp/triagefactory-runs/"+taskID); err != nil {
			t.Fatalf("SetWorktreePathSystem: %v", err)
		}
		if _, err := HolderPark(store, ctx, orgID, parked, db.ParkIdle()); err != nil {
			t.Fatalf("park: %v", err)
		}
		seed.SetSnapshotState(t, taskID, domain.WorkspaceSnapshotWritten)
		...
		if got := evictableFor(t, store, ctx, time.Now().Add(time.Hour), taskID); got != nil {
			t.Errorf("key %s is evictable while a sibling conversation holds a live claim; the tree would vanish under a running agent", taskID)
		}
		has, err := store.HasActiveClaimForTaskSystem(ctx, orgID, taskID)
		if err != nil || !has {
			t.Fatalf("HasActiveClaimForTaskSystem with a live sibling = %v (err %v), want true", has, err)
		}
		...
		if got := evictableFor(t, store, ctx, time.Now().Add(time.Hour), taskID); got == nil {
			t.Errorf("key %s is not evictable once every conversation is at rest and unclaimed", taskID)
		}
		has, err = store.HasActiveClaimForTaskSystem(ctx, orgID, taskID)
		if err != nil || has {
	...
	t.Run("ListEvictableWorkspaces_SkipsRowsWithNoWorktreePath", func(t *testing.T) {
		...
		seed.SetSnapshotState(t, wsTask, domain.WorkspaceSnapshotWritten)
		if got := evictableFor(t, store, ctx, time.Now().Add(time.Hour), wsTask); got != nil {
			t.Errorf("key %s with no recorded worktree_path enumerated as %+v; it names no tree to evict", wsTask, got)
		}
	})
	...
	t.Run("TerminalWrites_ReleaseActiveClaimWithOutcome", func(t *testing.T) {
		...
		t.Run("Complete_failed", func(t *testing.T) {
			conversationID := stage(t)
			if _, err := HolderComplete(store, ctx, orgID, conversationID, "failed", 0, 0, 0, "", "", "", ""); err != nil {
				t.Fatalf("Complete: %v", err)
			}
		...
		t.Run("MarkFailedIfActive", func(t *testing.T) {
			conversationID := stage(t)
			if ok, err := HolderMarkFailed(store, ctx, orgID, conversationID, ""); err != nil || !ok {
				t.Fatalf("MarkFailedIfActive: ok=%v err=%v", ok, err)
			}
			assertReleased(t, conversationID, "failed")
		})
		...
		t.Run("RefusedWrite_LeavesClaimActive", func(t *testing.T) {
			...
			if ok, _ := HolderMarkFailed(store, ctx, orgID, conversationID, ""); ok {
	...
	t.Run("SetActiveClaimPhaseSystem_NoOpOnReleasedClaim", func(t *testing.T) {
		...
		claim, err := store.SetExecutorSystem(ctx, orgID, conversationID, "exec-phase-rel", 1)
		if err != nil {
			t.Fatalf("SetExecutorSystem: %v", err)
		}
	...
	// The refusal half, on both dialects: the engagement's own settling park
	// releases its claim, and anything it writes after that — a late stream
	// row, a teardown that did not notice — is a write from a claim that is
	// settled history. Every fenced write must then refuse rather than land —
	...
	t.Run("ClaimFencedWrites_ReleasedClaimRefusesEveryWrite", func(t *testing.T) {
		...
		// The settling park: parks the row and releases the claim.
		if flipped, err := store.ParkOpenForClaimSystem(ctx, orgID, conversationID, claimID, db.ParkStopped(domain.ParkReasonUserCancelled, "")); err != nil || !flipped {
			t.Fatalf("parkForClaimSystem (the stop) = (%v, %v), want (true, nil)", flipped, err)
		}
	...
	t.Run("ParkOpenForClaimSystem_ParksAndReleasesLikeItsUnfencedTwin", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("parkForClaimSystem: %v", err)
		}
		if !ok {
	...
	t.Run("List_StatusFilterReadsTheDisplayLadder", func(t *testing.T) {
		...
		if _, err := HolderPark(store, ctx, orgID, queued, db.ParkIdle()); err != nil {
			t.Fatalf("park: %v", err)
		}
	...
	t.Run("List_QueuePositionIsTheOrgLocalPlaceInLine", func(t *testing.T) {
		...
		if _, err := HolderPark(store, ctx, orgID, second, db.ParkIdle()); err != nil {
			t.Fatalf("park: %v", err)
		}
		got = positions()
	...
	t.Run("HasLiveConversationForTask", func(t *testing.T) {
		...
		if _, err := HolderPark(store, ctx, orgID, manualID, db.ParkStopped(domain.ParkReasonUserCancelled, "")); err != nil {
			t.Fatalf("park: %v", err)
		}
		if has, _ := store.HasLiveConversationForTask(ctx, orgID, taskID); !has {
	...
	t.Run("EntitiesWithOpenConversations_FiltersByStatus", func(t *testing.T) {
		...
		if _, err := HolderPark(store, ctx, orgID, convA, db.ParkIdle()); err != nil {
...
// share the backend.
func evictableFor(t *testing.T, store db.ConversationStore, ctx context.Context, cutoff time.Time, taskID string) *domain.EvictableWorkspace {
	t.Helper()
...
// left for a subtest to vary is the snapshot state. Returns the task id — the
// snapshot key.
...
func reapKeysContain(keys []domain.SnapshotReapKey, taskID string) bool {
	for _, k := range keys {
		if k.TaskID == taskID {
			return true
		}
...
// than the conversation's, and for the workspace sweeps, which key by the task.
func seedConversationWithBlueprintForTest(t *testing.T, orgID string, seed ConversationSeeder, status string) (conversationID, blueprintRunID, taskID string) {
	t.Helper()
	_ = orgID

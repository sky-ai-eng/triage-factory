...
type MarketplaceStore interface {
	...
	// the publish flow's duplicate-source check and the editor's by-source
	// badge lookup. Status-blind on purpose: the
	// marketplace_listings_source_active partial unique index is
	// published-only, so a delisted row does not count against it at the DB
	// layer, and a read that mirrored the index would let a caller mint a
	// second listing for a source whose first one was delisted. Reading both
	// states is what routes that caller to relist instead. Returns the full

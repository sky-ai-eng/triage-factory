...
func TestPlacementClaim_TierOneExclusiveToOwner(t *testing.T) {
	...
	runID := enqueuePgRunPreferred(t, h, stores, orgID, userID, "exec-a")

	// The enqueue actually persisted the stamp.
	if got, ok := readPreferred(t, h, runID); !ok || got != "exec-a" {
		t.Fatalf("preferred_executor_id = (%q, %v), want exec-a", got, ok)
	}

	// A non-owner (B) must NOT claim a fresh, live-owned run.
	got, err := stores.RunQueue.ClaimNextRun(ctx, "exec-b", 1, placementClaimCfg)
	if err != nil {
	...
	// The owner claims it.
	got, err = stores.RunQueue.ClaimNextRun(ctx, "exec-a", 1, placementClaimCfg)
	if err != nil || got == nil || got.ID != runID {
		t.Fatalf("A claim = (%+v, %v), want run %s", got, err, runID)
	}
}
...
func TestPlacementClaim_AgesToSpillover(t *testing.T) {
	...
	runID := enqueuePgRunPreferred(t, h, stores, orgID, userID, "exec-a")
	backdatePgRunStarted(t, h, runID, 45*time.Second) // > 20s aging window
...
func TestPlacementClaim_DrainingAndGatedPreferredSpill(t *testing.T) {
	...
	t.Run("draining", func(t *testing.T) {
		...
		runID := enqueuePgRunPreferred(t, h, stores, orgID, userID, "exec-drain")
		got, err := stores.RunQueue.ClaimNextRun(ctx, "exec-helper", 1, placementClaimCfg)
		if err != nil || got == nil || got.ID != runID {
	...
	t.Run("gated", func(t *testing.T) {
		...
		runID := enqueuePgRunPreferred(t, h, stores, orgID, userID, "exec-gated")
		got, err := stores.RunQueue.ClaimNextRun(ctx, "exec-helper", 1, placementClaimCfg)
		if err != nil || got == nil || got.ID != runID {
...
func TestPlacementClaim_NullPreferredClaimableImmediately(t *testing.T) {
	...
	registerLiveExecutor(t, stores, "exec-a")
	runID := enqueuePgRunPreferred(t, h, stores, orgID, userID, "") // NULL preferred
...
func TestPlacementClaim_DisabledIgnoresPreferred(t *testing.T) {
	...
	got, err := stores.RunQueue.ClaimNextRun(ctx, "exec-other", 1, db.ClaimPlacement{})
	if err != nil || got == nil || got.ID != runID {
...
func TestPlacementClaim_RequeueClearsPreferred(t *testing.T) {
	...
	claimed, err := stores.RunQueue.ClaimNextRun(ctx, "exec-a", 1, placementClaimCfg)
	...
	if got, ok := readPreferred(t, h, runID); ok {

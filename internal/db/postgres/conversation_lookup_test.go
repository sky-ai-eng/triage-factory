...
func TestConversationStore_Postgres_LookupOrgForRunSystem_ReturnsRealOrgID(t *testing.T) {
	...
	got, err := stores.Conversations.LookupOrgForConversationSystem(ctx, conversationID)
	if err != nil {
		t.Fatalf("LookupOrgForConversationSystem: %v", err)
	}
	if got != orgID {
		t.Errorf("LookupOrgForConversationSystem = %q; want %q (the real Postgres org UUID, NOT the local sentinel)", got, orgID)
	}
}

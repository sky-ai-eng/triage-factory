...
func TestRunWorktreeStore_Postgres_InsertRingsCredDoorbell(t *testing.T) {
	...
	row := domain.RunWorktree{RunID: runID, RepoID: "sky-ai-eng/other-repo", Path: "/runs/" + runID + "/sky-ai-eng/other-repo/@default", Ref: "@default"}
	inserted, _, err := stores.RunWorktrees.InsertSystem(ctx, orgID, row)
	if err != nil || !inserted {
		t.Fatalf("InsertSystem: inserted=%v err=%v", inserted, err)
	}
	got := awaitCredRequest(t, msgs)
	if got.OrgID != orgID || got.RunID != runID {
		t.Errorf("doorbell = %+v, want org %s run %s", got, orgID, runID)
	}
...
func TestRunWorktreeStore_Postgres_RolledBackInsertRingsNoDoorbell(t *testing.T) {
	...
	inserted, _, err := txStores.RunWorktrees.Insert(ctx, orgID, domain.RunWorktree{
		RunID: runID, RepoID: "sky-ai-eng/other-repo", Path: "/runs/x", Ref: "@default",
	})
	if err != nil || !inserted {

...
// CompleteForClaimSystem is the terminal write: the fence, the cost
// settlement, the claim release and the flip, in one transaction. The claim it
// releases is resolved as the conversation's active claim — which the fence
// has just proven to be claimID, since only one claim per conversation can be
// unreleased at a time.
//
// The settlement runs before the flip because the flip RETURNINGs derived
// columns (total_cost_usd, duration_ms, num_turns, executor_id) read from
...
// settleCompletionCostAndClaim does everything the terminal write needs to
// happen BEFORE the conversation flip: locate the active claim, settle the
// cost lump onto the messages ledger, and release the claim with its
// telemetry. Split out from the conversations UPDATE (completeConversationFlip)
// so it can run first — see CompleteForClaimSystem for why the order is
// load-bearing.
...
func completeConversationFlip(ctx context.Context, q queryer, orgID, conversationID, status, resultSummary, outcome, outcomeReason, failureKind string) (*domain.Conversation, error) {
	...
	// lump on the invocation's last message row above. A resume's terminal
...
// ParkOpenForClaimSystem is the only park: the fence, the flip and the claim
// release in one transaction, so a park and its release commit together or
// not at all.
...
//
// A pending stop decides the reason whatever the caller passed, because the
// intent is what records who asked, and every path that delivers a stop —
...
func (s *conversationStore) ListReapableSnapshotKeysSystem(ctx context.Context, cutoff time.Time) ([]domain.SnapshotReapKey, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&k.OrgID, &k.TaskID); err != nil {
			return nil, err
		}
		keys = append(keys, k)
	}
...
func (s *conversationStore) ListEvictableWorkspacesSystem(ctx context.Context, cutoff time.Time) ([]domain.EvictableWorkspace, error) {
	...
	// claims.
	rows, err := s.admin.QueryContext(ctx, `
		SELECT DISTINCT c.org_id, c.task_id, c.worktree_path
		FROM conversations c
	...
	for rows.Next() {
		var orgID, taskID, path string
		if err := rows.Scan(&orgID, &taskID, &path); err != nil {
			return nil, err
		}
		// The ORDER BY groups a key's paths adjacently, so one pass folds them.
		if n := len(out); n > 0 && out[n-1].OrgID == orgID && out[n-1].TaskID == taskID {
...
func (s *conversationStore) SetSystemBlockForClaimSystem(ctx context.Context, orgID, conversationID, claimID, block string) (*domain.Conversation, error) {
	...
	err := inTx(ctx, s.admin, func(q queryer) error {
		...
		r, err := writeConversationReturning(ctx, q, `
			UPDATE conversations SET system_block = $1 WHERE org_id = $2 AND id = $3
			RETURNING *
		`, block, orgID, conversationID)
		if err != nil {
			return err
		}
		result = r
...
func (s *conversationStore) SystemBlockSystem(ctx context.Context, orgID, conversationID string) (string, error) {
	var block string
	err := s.admin.QueryRowContext(ctx,
		`SELECT system_block FROM conversations WHERE org_id = $1 AND id = $2`, orgID, conversationID).Scan(&block)
	if errors.Is(err, sql.ErrNoRows) {
...
// MarkFailedIfActiveForClaimSystem is the infra-failure terminal behind the
...
func (s *conversationStore) RequestStopSystem(ctx context.Context, orgID, conversationID, by, signalTarget string, reason domain.ParkReason) (bool, error) {
	...
	err := inTx(ctx, s.admin, func(q queryer) error {
		...
		`, orgID, conversationID, string(domain.ConversationSignalCancel), signalTarget)
		return err
	})
	if err != nil {
...
// must keep warm — conversations parked `open` or left mid-flight (stored
// status NULL) whose owning blueprint_run is still 'running'. One under an
// already-terminal blueprint_run is NOT resumable (every resume path gates on
// cr.Status == running), so its worktree must NOT be preserved: preserving it
// would leave a checked-out branch on disk that the boot reconcile then
// orphans by cancelling the row, reviving the "refusing to fetch into a
// branch checked out in a worktree" loop. Admin pool — the startup sweep has

...
// pgSequenceScaffold resets the harness and builds the scaffold every
...
// a bare insert took.
func pgSequenceScaffold(t *testing.T, h *pgtest.Harness) (db.BlueprintStore, dbtest.BlueprintSequenceScaffold) {
	t.Helper()
	h.Reset(t)
	stores := pgstore.New(h.AdminDB, h.AdminDB, pgtest.SecretKey)
	ctx := context.Background()

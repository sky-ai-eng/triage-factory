...
func newPgConversationSeeder(conn *sql.DB, orgID, userID, agentID, promptID string) dbtest.ConversationSeeder {
	...
	return dbtest.ConversationSeeder{
		...
		BackdateQueuedAt: func(t *testing.T, conversationID string, age time.Duration) {
			t.Helper()
			if _, err := conn.Exec(
				`UPDATE conversations SET queued_at = now() - make_interval(secs => $2) WHERE id = $1`,
				conversationID, age.Seconds()); err != nil {
		...
		BlueprintRun: func(t *testing.T, taskID string) string {
			...
			if _, err := conn.Exec(`
				UPDATE blueprint_runs SET status = 'completed', completed_at = now()

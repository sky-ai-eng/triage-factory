...
--
-- Durable router event queue (transactional outbox), adopting the shared
-- work-item block (internal/db/workitem). The events audit row and the queue
-- row are written atomically at ingest, and the router's worker claims rows
-- under the shared lease contract: status is ready | leased | done | parked |
-- cancelled, a claim charges the attempt budget and stamps a lease whose
-- generation fences every later write, an expired lease is claimable by the
-- next claim with no reset or sweeper, and a failed attempt returns to ready
-- with a retry time or parks for an operator. Delivery is at-least-once and
-- routing is replay-safe behind its domain fences. unique_key is set only for
-- the close obligation, which one entity holds while one is ready, leased or
-- parked. The five columns after the block are this kind's own.
--
-- Admin-pool wired with org_id bound per statement; the event_queue_all
-- policy below is defense-in-depth, and it is FOR ALL because admission's
-- conflict arm and every guarded write are UPDATEs.
--

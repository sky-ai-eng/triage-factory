...
type RunSignalStore interface {
	...
	Insert(ctx context.Context, orgID, runID string, kind domain.RunSignalKind, payload, target string) (id int64, err error)
	...
	// contract.
	ListUnackedForTarget(ctx context.Context, target string) ([]domain.RunSignal, error)

...
func TestBlueprintStore_SQLite_Sequence(t *testing.T) {
	dbtest.RunBlueprintSequenceConformance(t, sqliteSequenceScaffold)
}

// sqliteSequenceScaffold builds one clean database and the scaffold every
// blueprint_runs suite stages through. Shared because a run is only minted by
// the firing door now, so a suite about runs needs a firing's whole fixture —
// a blueprint, a trigger, an event, a prompt and a step — not just the two ids
// a bare insert took.
func sqliteSequenceScaffold(t *testing.T) (db.BlueprintStore, dbtest.BlueprintSequenceScaffold) {
	t.Helper()

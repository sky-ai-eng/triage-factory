...
func TestRunSignalStore_SQLite_EveryMethodReturnsErrNotApplicableInLocal(t *testing.T) {
	...
	ctx := t.Context()
	rs := stores.RunSignals

	const orgID = runmode.LocalDefaultOrgID
	const runID = "some-run"
	const target = "some-executor"

	if _, err := rs.Insert(ctx, orgID, runID, domain.RunSignalCancel, "", target); !errors.Is(err, db.ErrNotApplicableInLocal) {
	...
	if err := rs.Ack(ctx, 1, domain.RunSignalAckOK); !errors.Is(err, db.ErrNotApplicableInLocal) {

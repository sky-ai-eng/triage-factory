...
func (s *eventQueueStore) Enqueue(ctx context.Context, orgID string, evt domain.Event, traceparent string) (string, error) {
	...
	if err := db.InTx(ctx, s.conn, func(tx *sql.Tx) error {
		...
		return insertQueueRow(ctx, tx, id, evt, traceparent, nil)
	}); err != nil {
		return "", err
	}
	return id, nil
...
// stores NULL.
func insertQueueRow(ctx context.Context, q queryer, eventID string, evt domain.Event, traceparent string, entityPollSeq *int64) error {
	var entityID any
	...
	_, err := q.ExecContext(ctx, `
		INSERT INTO event_queue (event_id, entity_id, event_type, status, traceparent, entity_poll_seq)
		VALUES (?, ?, ?, 'pending', NULLIF(?, ''), ?)
	`, eventID, entityID, evt.EventType, traceparent, entityPollSeq)
	return err
}
...
func (s *eventQueueStore) EnqueueBatchWithSnapshotCAS(ctx context.Context, orgID, entityID, snapshotJSON string, expectedPollSeq int64, events []domain.Event, traceparents []string) (bool, []string, error) {
	...
	if err := db.InTx(ctx, s.conn, func(tx *sql.Tx) error {
		...
		for i, evt := range events {
			...
			if err := insertQueueRow(ctx, tx, id, evt, db.TraceparentAt(traceparents, i), &judgedAt); err != nil {
				return err
			}
			ids = append(ids, id)
		}
...
func scanSqliteQueuedEvent(row *sql.Row) (*domain.QueuedEvent, error) {
	...
	applyQueuedEventNulls(&qe, claimedAt, processedAt, pollSeq)
	return &qe, nil
}
...
func applyQueuedEventNulls(qe *domain.QueuedEvent, claimedAt, processedAt sql.NullTime, pollSeq sql.NullInt64) {
	if claimedAt.Valid {
		t := claimedAt.Time
		qe.ClaimedAt = &t
	}

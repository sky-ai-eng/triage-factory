...
func TestConversationQueueStore_SQLite_EnqueueClaim(t *testing.T) {
	...
	if got == nil {
		t.Fatal("ClaimNextConversation returned nil for a queued run")
	}
...
func TestConversationQueueStore_SQLite_RequeueAndReset(t *testing.T) {
	...
	if err := stores.ConversationQueue.RequeueConversation(ctx, org, "rqr-run-0", "transient setup error"); err != nil {
...
func TestConversationQueueStore_SQLite_RequeueFromSetupPhase(t *testing.T) {
	for _, phase := range domain.AllClaimPhases() {
		t.Run(phase, func(t *testing.T) {
			...
			if err := stores.ConversationQueue.RequeueConversation(ctx, org, "rqs-run-0", "workspace setup: boom"); err != nil {
				t.Fatalf("RequeueConversation: %v", err)
			}
...
func TestConversationQueueStore_SQLite_ResetLeavesDormantAlone(t *testing.T) {
	...
	if n != 0 {
		t.Fatalf("ResetProcessingConversations reset %d rows, want 0 (dormant run must stay parked)", n)
	}
...
func TestConversationQueueStore_SQLite_RejectsNonLocalOrg(t *testing.T) {
	...
	if err := stores.ConversationQueue.EnqueueConversation(ctx, bogusOrg, domain.Conversation{}); err == nil {
		t.Errorf("EnqueueConversation with non-local orgID should error")
	}
	if err := stores.ConversationQueue.RequeueConversation(ctx, bogusOrg, "r", "x"); err == nil {
...
func TestConversationQueueStore_SQLite_QueuedAtStamps(t *testing.T) {
	...
	if err := stores.ConversationQueue.RequeueConversation(ctx, org, "rq-dwell-run", "transient setup error"); err != nil {
		t.Fatalf("RequeueConversation: %v", err)
	}
	...
	dbtest.RunClaimPredicateConformance(t, func(t *testing.T) dbtest.ClaimPredicateHarness {
		...
		return dbtest.ClaimPredicateHarness{
			...
			EnqueueUnindexed: func(t *testing.T) error {
				t.Helper()
				return stores.ConversationQueue.EnqueueConversation(ctx, org, domain.Conversation{

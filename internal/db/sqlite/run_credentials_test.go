...
func seedSQLiteRunForCredentials(t *testing.T, conn *sql.DB) string {
	t.Helper()
	id := uuid.New().String()
	if _, err := conn.Exec(
...
// TestRunCredentialsStore_PutGetDelete pins the basic round trip.
func TestRunCredentialsStore_PutGetDelete(t *testing.T) {
	conn := openSQLiteForTest(t)
	stores := sqlitestore.New(conn)
	ctx := context.Background()
	runID := seedSQLiteRunForCredentials(t, conn)
...
// TestRunCredentialsStore_PutNeverRegressesBootEpoch pins the CAS guard: a
// write for an OLDER boot_epoch arriving after a NEWER one (the slow-
// provision-races-a-reclaim window) must be a silent no-op, never an
// overwrite — the fresher claim's bundle must survive. A same-epoch write
// (a refresh for the SAME still-live claim) must still apply normally.
func TestRunCredentialsStore_PutNeverRegressesBootEpoch(t *testing.T) {
	conn := openSQLiteForTest(t)
	stores := sqlitestore.New(conn)
	ctx := context.Background()
	runID := seedSQLiteRunForCredentials(t, conn)

...
// a given base, at the run's default location runDir(conversationID). If
...
func CreateForBranch(ctx context.Context, owner, repo, cloneURL, baseBranch, featureBranch, conversationID string, opts ...CloneOption) (string, error) {
	wtDir, err := makeWorktreeDir(conversationID)
	if err != nil {
		return "", err
	}
	return createBranchWorktreeAt(ctx, owner, repo, cloneURL, baseBranch, featureBranch, conversationID, wtDir, resolveCloneOptions(opts).auth)
}
...
func CreateForBranchInRoot(ctx context.Context, owner, repo, cloneURL, baseBranch, featureBranch, conversationID, runRoot string) (string, error) {
	if runRoot == "" {
	...
	// the in-sandbox path.
	return createBranchWorktreeAt(ctx, owner, repo, cloneURL, baseBranch, featureBranch, conversationID, wtDir, CloneAuth{})
}
...
func CreateForCheckoutInRoot(ctx context.Context, owner, repo, cloneURL, ref, conversationID, runRoot string, opts ...CloneOption) (string, error) {
	if runRoot == "" {
	...
	if runmode.Current() == runmode.ModeMulti {
		return createCheckoutCloneAt(ctx, owner, repo, cloneURL, ref, conversationID, wtDir, auth)
	}
	return createCheckoutWorktreeAt(ctx, owner, repo, cloneURL, ref, conversationID, wtDir, auth)
}
...
func createCheckoutWorktreeAt(ctx context.Context, owner, repo, cloneURL, ref, conversationID, wtDir string, auth CloneAuth) (string, error) {
	...
	if err := addExcludesOrRollback(conversationID, wtDir); err != nil {
		return "", err
	}
...
// run-namespaced branch (triagefactory/<conversationID>/checkout) carries the tip
// through the clone and is deleted from both the clone and the bare afterwards.
func createCheckoutCloneAt(ctx context.Context, owner, repo, cloneURL, ref, conversationID, wtDir string, auth CloneAuth) (string, error) {
	mu := lockRepo(owner, repo)
	mu.Lock()
	defer mu.Unlock()
	...
	tmpBranch := fmt.Sprintf("triagefactory/%s/checkout", conversationID)
...
// checkout is the run-namespaced triagefactory/<conversationID>/pr-<n> while
...
func createBranchWorktreeAt(ctx context.Context, owner, repo, cloneURL, baseBranch, featureBranch, conversationID, wtDir string, auth CloneAuth) (string, error) {
	mu := lockRepo(owner, repo)
	mu.Lock()
	defer mu.Unlock()
...
func addExcludesOrRollback(conversationID, wtDir string) error {
	if err := writeLocalExcludes(wtDir); err != nil {

...
func TestCreateForCheckoutInRoot_SelfContainedClone_MultiMode(t *testing.T) {
	...
	const conversationID = "checkout-run-multi"
	runRoot := t.TempDir()
	wtPath, err := CreateForCheckoutInRoot(context.Background(), "acme", "repo", cloneURL, "", conversationID, runRoot,
	...
	staging := "triagefactory/" + conversationID + "/checkout"

...
func TestTerminalCloseSet_CoversTaskTypes(t *testing.T) {
	...
	// The deletion set is checked separately because it spares a different
	// type: a completion can leave a jira:issue:completed task standing, a
	// deletion cannot leave anything.
	covers("jira:issue:", jiraIssueDeletedCloseTypes(), domain.EventJiraIssueDeleted)
}

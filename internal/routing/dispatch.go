...
// flip a terminating event performs), handler matching, the task upsert, and
// trigger firing (plus the delegation tail behind it): each one is load-bearing
// for whether the right task exists, the resolved ones are gone, and the right
// run started — and none of them is recoverable by any other path in time. The
// tracker's snapshot-diff is forward-only, so a dropped event is simply never
// re-derived; a lost close is worse still, since a terminating transition emits
// exactly once and nothing else reconciles an entity whose close write failed
// (only the periodic terminal reconciler does, and at sweep latency). The drain
// worker answers an error by requeueing the row for another attempt (see

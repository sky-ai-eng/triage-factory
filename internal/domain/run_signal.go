...
// ConversationSignalKind discriminates the five cross-pod run-control operations
...
const (
	...
	// ConversationSignalInterrupt stops the run's current turn, leaving the process
	// alive for further input.
	ConversationSignalInterrupt ConversationSignalKind = "interrupt"
	// ConversationSignalSteer delivers a free-form user message to a live run.
	// Payload: {"text": "..."}.
...
const (
	...
	// ConversationSignalAckGone means the owner found the run no longer live by the

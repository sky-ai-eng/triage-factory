...
func installToolchain(ctx context.Context, rootfsDir string) error {
	...
	if err := installPnpm(ctx, rootfsDir); err != nil {
...
// Explicit rather than inherited from the host process: npm's entry
// point has a `#!/usr/bin/env node` shebang, so PATH must resolve to a
// `node` that exists inside the chroot's own filesystem view (the
// apk-installed /usr/bin/node), not wherever the host happens to keep
// its own Node. HOME=/root gives npm a writable cache directory for the
// duration of the global install (installPnpm removes it afterward so
// it doesn't freeze into the shared rootfs).
var chrootToolchainEnv = []string{
	"PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
	"HOME=/root",
}

// installPnpm installs the exact pnpm release the frontend pins
// (pnpmVersion) as a global npm package inside the chroot, so
// /usr/bin/pnpm exists on PATH in the cached rootfs. pnpm is not an apk
// package, so it is layered on top of the apk-installed npm here.
//
// This runs at rootfs-BUILD time, when the host has ordinary internet
// egress (the same path the apk-add call above just used) — not at
// agent-RUN time, when the sandboxed agent's egress is locked to its
// own gateway IP (applyEgressPolicy) and only reaches the allowlisted
// package registries. Baking pnpm now means the agent's frontend build
// finds it already on PATH; because pnpm >= 10 self-manages the
// packageManager field, a repo pinning this exact version runs the
// baked binary in place with zero network, and a repo pinning a
// different version fetches that one through the egress proxy.
//
// --prefix=/usr is explicit for determinism — alpine node derives the
// same default, but pin it rather than lean on the derivation. The
// install leaves an npm cache under HOME (/root/.npm); a global install
// is HOME-independent once written, so nothing at runtime needs the
// cache — remove it so it doesn't bloat the frozen rootfs.
func installPnpm(ctx context.Context, rootfsDir string) error {
	install := exec.CommandContext(ctx, "chroot", rootfsDir,
		"/usr/bin/npm", "install", "-g", "--prefix=/usr",
		"--no-audit", "--no-fund", "pnpm@"+pnpmVersion)
	install.Env = chrootToolchainEnv
	var stderr bytes.Buffer
	install.Stderr = &stderr
	if err := install.Run(); err != nil {
		return fmt.Errorf("npm install -g pnpm@%s: %w (stderr: %s)", pnpmVersion, err, strings.TrimSpace(stderr.String()))
	}

	if err := os.RemoveAll(filepath.Join(rootfsDir, "root", ".npm")); err != nil {
		return fmt.Errorf("clean npm cache: %w", err)
	}
	return nil
}

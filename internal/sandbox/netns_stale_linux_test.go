...
// namespace. Skipping (rather than failing) matches the rest of this package's
// privileged tests: the same suite has to run on a stripped-down dev box.
func requireNetnsCapableHost(t *testing.T, probeName string) {
	t.Helper()
	if _, err := exec.LookPath("ip"); err != nil {
		t.Skip("iproute2 not installed; skipping netns test")
	}
	ctx := context.Background()
	if err := runIP(ctx, "netns", "add", probeName); err != nil {
		t.Skipf("this environment does not permit netns creation (needs root/CAP_SYS_ADMIN): %v", err)
	}
	t.Cleanup(func() { _ = runIPNoErr(context.Background(), "netns", "delete", probeName) })
}

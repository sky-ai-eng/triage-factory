...
func TestRun_RescoresCrashResidue(t *testing.T) {
	...
	// OnScoringCompleted announces the IDs the cycle scored, and the score
	// write admitted a re-evaluation for each: the recovered rows have to
	// be among both, or their deferred triggers still never fire.
	...
	for _, id := range residue {
		if !inCallback[id] {
			t.Errorf("recovered task %s missing from OnScoringCompleted", id)
		}
		if got := countReDeriveRows(t, database, id, "ready"); got != 1 {
			t.Errorf("recovered task %s has %d ready re-evaluation rows, want 1", id, got)
		}
	}
}

// TestRun_ScoresCommitAdmitsTheReEvaluationBeforeTheCompletionCallback pins
// the handoff to the re-derive worker: by the time OnScoringCompleted runs —
// synchronously, inside the cycle, with no goroutine of the runner's — each
// scored task's re-evaluation row is already committed at the revision the
// scores carry, so the wake the hook sends finds work to claim.
func TestRun_ScoresCommitAdmitsTheReEvaluationBeforeTheCompletionCallback(t *testing.T) {
	ctx := context.Background()
	database := newScoringTestDB(t)
	stores := sqlitestore.New(database)
	...
	var calls int
	r := NewRunner(stores.Scores, nil, runmode.LocalDefaultOrgID, nil, nil, nil, nil, fixedModel, RunnerCallbacks{
		OnScoringCompleted: func(_ context.Context, _ string, scored []string) {
			calls++
			if len(scored) != len(ids) {
				t.Errorf("callback announced %d tasks, want %d", len(scored), len(ids))
			}
			for _, id := range scored {
				if got := countReDeriveRows(t, database, id, "ready"); got != 1 {
					t.Errorf("task %s has %d ready re-evaluation rows inside the callback, want 1 already committed", id, got)
				}
				var rev int64
				if err := database.QueryRow(`SELECT requested_revision FROM task_rederive_queue WHERE task_id = ?`, id).Scan(&rev); err != nil || rev != 1 {
					t.Errorf("task %s re-evaluation requested_revision = %d (%v), want 1", id, rev, err)
				}
			}
		},
	})
	...
	if calls != 1 {
		t.Fatalf("OnScoringCompleted ran %d times by the time the cycle returned, want exactly once, synchronously", calls)
	}
}
...
func countReDeriveRows(t *testing.T, database *sql.DB, taskID, status string) int {
	t.Helper()
	var n int
	if err := database.QueryRow(`SELECT COUNT(*) FROM task_rederive_queue WHERE task_id = ? AND status = ?`, taskID, status).Scan(&n); err != nil {
		t.Fatalf("count re-evaluation rows of %s: %v", taskID, err)
	}
	return n
}

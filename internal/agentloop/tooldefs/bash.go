...
//
// The two summary params exist because a shell command is the only argument
// in this registry that is not already legible to a person — a path, a
// pattern, a directory each describe themselves, while
...
// tense is authored rather than derived: nothing transforms one into the
// other.
//
// Nothing executes them, but the jail declares them identically all the same
// and validates them like any other argument: a wrong-typed summary fails
// the call rather than being quietly dropped. What the model is told and
// what the jail accepts are one schema, with no carve-out for the parts that
// only render.
var Bash = Tool{
	Name:        "bash",

// The one table every drawn mark in the app is read from — the shell rail's
// `Ico` and the board card's `Glyph` both take their paths here, so a shape
// the product already has is never drawn twice. A 16px box, every path
// authored against that grid; see glyphs.tsx for why lucide-react is not used.
//
// Its own module rather than a second export of glyphs.tsx, so that file keeps
